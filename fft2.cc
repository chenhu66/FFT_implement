...
template<typename T>
int valid_vector(const std::vector<std::complex<T>> & lhs, const std::vector<std::complex<T>> & rhs, T delta = (T)0.001){
...
template<typename T>
void rand_vec(std::vector<std::complex<T>> & seq){
    ...
    for(i=0;i<seq.size();i++){
        seq[i] = std::complex<T>(dist(mt), dist(mt));
    }
}
template<typename T>
...
template<typename T>
void fft_naive(const std::vector<std::complex<T>> & t_seq, std::vector<std::complex<T>> & f_seq, size_t length=0){
    ...
    size_t fft_n = length;
    size_t k;
    std::vector<std::complex<T>> seq = t_seq;
    ...
    for(k=0;k<fft_n;k++){
        size_t n;
        std::complex<T> omega_k = omega_func(fft_n, k);
        std::complex<T> A_k;
        for(n=0;n<fft_n;n++){
...
template<typename T>
void ifft_naive(const std::vector<std::complex<T>> & f_seq, std::vector<std::complex<T>> & t_seq, size_t length=0){
    ...
    std::vector<std::complex<T>> seq = f_seq;
    if(length > f_seq.size()){
        for(size_t i=0; i< (length - f_seq.size()); i++ ){
            seq.push_back(std::complex<T>());
        }
    }
    for(k=0;k<fft_n;k++){
        size_t n;
        std::complex<T> omega_k_inverse = omega_func_inverse(fft_n, k);
        std::complex<T> a_k;
        for(n=0;n<fft_n;n++){
...
template<typename T>
void fft_cooley_tukey(std::complex<T> * seq, size_t length)
{
    if(length==1){
        //f_seq[0] = t_seq[0];
        return;
    ...
    * 
    */
    std::vector<std::complex<T>> omega_list;   // pre-compute omega, and index to it later
...
template<typename T>
void ifft_cooley_tukey(std::complex<T> * seq, size_t length){
    if(length == 1)
        return;
    assert( ( (length & (length - 1)) == 0 ) && "current only length power of 2");
    bit_reverse_radix2(seq, length);
...
template<typename T>
void _fft_cooley_tukey_r(std::complex<T> * seq, size_t length, bool is_inverse_fft){
    if(length == 1)
        return;
    assert( ( (length & (length - 1)) == 0 ) && "current only length power of 2");

    std::function<std::complex<T>(size_t,size_t)> omega_func;
    if(is_inverse_fft){
   ...
   for(size_t itr = 2; itr<=length; itr<<=1){
        ...
        for(size_t g=0;g<groups;g++){
            ...
            std::complex<T> & omega = omega_list[k];
            for(size_t s=0;s<stride;s++){
                //printf("W%d_%d(%d,%d-%d) ", itr, k, g, g*group_len+s, g*group_len+s+stride);
                std::complex<T> & a = seq[g*group_len+s];
                std::complex<T> & b = seq[g*group_len+s+stride];
                BUFL2(a, b, omega);
            }
        }
        //printf("\n");
    ...
    if(is_inverse_fft){
        for(size_t i=0;i<length;i++)
            seq[i] = seq[i]/(T)length;
    }
}
template<typename T>
void fft_cooley_tukey_r(std::complex<T> * seq, size_t length){
    _fft_cooley_tukey_r(seq, length,false);
}
template<typename T>
void ifft_cooley_tukey_r(std::complex<T> * seq, size_t length){
...
*/
template<typename T>
void fft_r2c(const T* t_seq, std::complex<T> * f_seq, size_t length, bool half_mode=false){
    ...
    f_seq[length/2] = std::complex<T>( std::real(seq[0])-std::imag(seq[0]), (T)0);
    if(!half_mode){
        for(size_t i=1;i<length/2;i++){
            f_seq[length-i] = std::conj(f_seq[i]);
        }
...
*/
template<typename T>
void ifft_c2r(const std::complex<T> * f_seq, T* t_seq, size_t length, bool half_mode=false){
...
template<typename T>
void fft_2d(std::complex<T>* seq, size_t seq_w, size_t seq_h)
{
    size_t i,j;
    for(i=0;i<seq_h;i++){
...
template<typename T>
void ifft_2d(std::complex<T>* seq, size_t seq_w, size_t seq_h)
{
    size_t i,j;
    for(i=0;i<seq_h;i++){
...
template<typename T>
void fft2d_r2c(const T* t_seq, std::complex<T> * f_seq, size_t seq_w, size_t seq_h, bool half_mode=false){
    size_t v_len = half_mode?(seq_h/2+1):seq_h; // vertical fft, then horizontal 
#if 0
    // vertical
    for(size_t w=0;w<seq_w;w++){
        T v[seq_h];
        std::complex<T> f_v[v_len];
    ...
    for(size_t w=0;w<seq_w;w++){
        T* v = new T[seq_h];
        std::complex<T>* f_v = new std::complex<T>[v_len];
        for(size_t h=0;h<seq_h;h++){
            v[h] = t_seq[h*seq_w+w];
        }
        fft_r2c(v, f_v, seq_h, half_mode);
    ...
    auto omega_func = [](size_t total_n, size_t k){
        return std::polar<T>((T)1, (-1*C_2PI*k/total_n));
    };
    ...
    for(size_t h=0;h<v_len;h++){
        ...
        std::vector<std::complex<T>> f_even;
        std::vector<std::complex<T>> f_odd;
        for(size_t w=0;w<seq_w/2;w++){
...
template<typename T>
void ifft2d_c2r(const std::complex<T> * f_seq, T* t_seq, size_t seq_w, size_t seq_h, bool half_mode=false){
    size_t v_len = half_mode?(seq_h/2+1):seq_h;

    std::vector<std::complex<T>> _seq;
    _seq.resize(seq_w*v_len);

    
    auto omega_func = [](size_t total_n, size_t k){
        return std::polar<T>((T)1, (C_2PI*k/total_n));
    };
    std::vector<std::complex<T>> omega;
    for(size_t w=0;w<seq_w/2;w++){
        omega.push_back(omega_func(seq_w, w));
    ...
    for(size_t h=0;h<v_len;h++){
        ...
        for(size_t w=0;w<seq_w/2;w++){
            _seq[h*seq_w+w] = (f_even[w]+f_odd[w]*omega[w])/(T)2;      // NOTICE, need divide 2
            _seq[h*seq_w+w+seq_w/2] = (f_even[w]-f_odd[w]*omega[w])/(T)2;
        }
    }

    // vertical
    for(size_t w=0;w<seq_w;w++){
        std::complex<T>* v = new std::complex<T>[v_len];
        T* t_v = new T[seq_h];
        for(size_t h=0;h<v_len;h++){
            v[h] = _seq[h*seq_w+w];
        }
...
void convolve_fft(const std::vector<T> & data, const std::vector<T> & filter, std::vector<T> &  dst, bool correlation = false){
    ...
    std::vector<std::complex<T>> seq_data;
    std::vector<std::complex<T>> seq_filter;
    ...
    std::vector<std::complex<T>> twiddle_for_conj;
    ...
    if(correlation){
        ...
        for(size_t i=0;i<padding_size;i++){
            ...
            seq_filter.insert(seq_filter.begin(),  std::complex<T>((T)0, (T)0));
        }
    }
    ...
    T* dst, bool correlation = false)
{
    ...
    if(correlation){
        ...
        seq_filter[0] = std::complex<T>(filter[(filter_h-1)*filter_w+filter_w-1], T(0));
        for(size_t i=0;i<filter_w-1;i++){
            seq_filter[pad_w+i+1] = std::complex<T>(filter[(filter_h-1)*filter_w+i], T(0));
        }
        for(size_t j=0;j<fft_len_h-1;j++){
            if(j<filter_h-1){
                seq_filter[(j+1)*fft_len_w] = std::complex<T>(filter[j*filter_w+filter_w-1], T(0));
                for(size_t i=0;i<filter_w-1;i++){
                    seq_filter[(j+1)*fft_len_w+pad_w+i+1] = std::complex<T>(filter[j*filter_w+i], T(0));
                }
            }
        }
    }
#else
    if(correlation){
        std::vector<std::complex<T>> tmp_filter;
        ...
        for(size_t j=0;j<fft_len_h;j++){
            if(j<filter_h){
                for(size_t i=0;i<filter_w;i++){
                    seq_filter[j*fft_len_h+i] = std::complex<T>(filter[j*filter_w+i], T(0));
                }
            }
        }
    }
    ...
    for(size_t j=0;j<fft_len_h;j++){
        if(j<data_h){
            for(size_t i=0;i<data_w;i++){
                seq_data[j*fft_len_h+i] = std::complex<T>(data[j*data_w+i], T(0));
            }
        }
    }
...
#if 0
    {
        std::vector<std::complex<d_type>> v;
        for(auto & it : filter)
...
int main(){
    std::cout << "fft2 using std::complex\n";
    ...
    for(size_t size = 2; size<=total_size; size *= 2){
        std::vector<std::complex<d_type>> t_seq;
        std::vector<std::complex<d_type>> f_seq;
        std::vector<std::complex<d_type>> t_seq_r;

        std::vector<std::complex<d_type>> seq_fwd;
        std::vector<std::complex<d_type>> seq_bwd;
        t_seq.resize(size);
        rand_vec(t_seq);
    ...
    for(size_t size = 2; size<=total_size; size *= 2){
        ...
        for(size_t ii=0;ii<size;ii++){
            t_seq.push_back(std::complex<d_type>(seq_fwd_real[ii],(d_type)0));
        }
        ...
        for(size_t ii=0;ii<size;ii++){
            seq_bwd.push_back(std::complex<d_type>(seq_bwd_real[ii],(d_type)0));
        }
        int ierr_cnt = valid_vector(t_seq_r, seq_bwd);
    ...
    for(size_t size = 2; size<=total_size; size *= 2){
        ...
        std::vector<std::complex<d_type>> t_seq_r_ex;

        std::vector<std::complex<d_type>>  t_seq_2;
